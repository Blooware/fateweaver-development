        ...
        connection.query("select title, first_name, second_name, email, added from fateweaver.tutors where id = ? and school_id = (select school_id from fateweaver.admins where cognito_id = ?)", [event.form.tutor_id, event.account.sub], function (err, results, fields) {
            ...
            if (results.length > 0) {
                ...
                connection.query("select id, name, description, added from fateweaver.tutor_groups where school_id = (select school_id from fateweaver.admins where cognito_id = ?)", [event.account.sub], function (err, results, fields) {
                    ...
                    connection.query("select id, name, description, added from fateweaver.tutor_groups where id in (select group_id from fateweaver.tutor_assigned_groups where tutor_id = ? and school_id = (select school_id from fateweaver.admins where cognito_id = ?)) and school_id = (select school_id from fateweaver.admins where cognito_id = ?)", [event.form.tutor_id, event.account.sub, event.account.sub], function (err, results, fields) {
                        if (err) {
                            console.log("Error getting tutor groups:", err);
                            context.succeed({
                                statusCode: 200,
                        ...
                        var tutorAssignedGroups = results;
                        ...
                        context.succeed({
                            ...
                            tutorAssignedGroups: tutorAssignedGroups,
