...
exports.handler = (event, context, callback) => {
   ...
   console.log(event.form.student_id);
    connection.query("select * from fateweaver.mentors where id = ? and school_id = (select school_id from fateweaver.admins where cognito_id = ?)", [event.form.mentorAssign.Mentors, event.account.sub], function (err, results, fields) {
        if (err) {
        ...
        if (results.length > 0) {
            connection.query("select * from fateweaver.students where id = ? and school_id = (select school_id from fateweaver.admins where cognito_id = ?)", [event.form.student_id, event.account.sub], function (err, results, fields) {
                ...
                if (results.length > 0) {
                    ...
                    connection.query("select * from fateweaver.mentor_assigned where student_id = ? and mentor_id = ? and school_id = (select school_id from fateweaver.admins where cognito_id = ?)", [event.form.student_id, event.form.mentorAssign.Mentors, event.account.sub], function (err, results, fields) {
                        if (err) {
                            console.log("Error getting tutor groups:", err);
                            context.succeed({
                                statusCode: 200,
