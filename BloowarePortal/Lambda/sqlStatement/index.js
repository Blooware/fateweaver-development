...
if(fateweaver.students.active = 1   ,(select DATE_FORMAT(date('1970-12-31 23:59:59') + interval -1* (
...
- (select sum(UNIX_TIMESTAMP(added)) from fateweaver.deactivation_log where active = 0)
...
,
if((select count(student_id) from fateweaver.deactivation_log where student_id = fateweaver.students.id ) = 1, 
...
) as ActiveTime from fateweaver.students 